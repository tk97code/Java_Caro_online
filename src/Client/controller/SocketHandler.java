...
public class SocketHandler {
    ...
    private void onReceiveCreateRoom(String received) {
        ...
        if (status.equals("failed")) { 
            //System.out.println("Room #" + roomId + " is existed");
            JOptionPane.showMessageDialog(RunClient.mainMenuScene, "Đã tồn tại phòng #" + roomId, "Lỗi", JOptionPane.ERROR_MESSAGE);
        } else if (status.equals("success")) {
            
            System.out.println("Room #" + roomId + " is created");
            RunClient.closeScene(RunClient.SceneName.CREATEROOM);
        }
        System.out.println("RECEIVED: " + received);
    }
    ...
    public void createRoomAction(String roomId) {
        String data = StreamData.Type.CREATE_ROOM.name() + ";" + roomId;
        sendData(data);
    }
    
    public void joinRoomAction(String roomId) {
        String data = StreamData.Type.JOIN_ROOM.name() + ";" + roomId;
        sendData(data);
    }
