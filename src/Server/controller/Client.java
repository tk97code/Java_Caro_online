...
public class Client implements Runnable {
    ...
    private void onReceiveCreateRoom(String received) {
        ...
        String result = "success";
        if (RunServer.roomManager.find(roomId) != null) {
            result = "failed";
            sendData(StreamData.Type.CREATE_ROOM.name() + ";" + roomId + ";" + result);
        } else {
            System.out.println("RRECEIVED: " + received);
            Room newRoom = RunServer.roomManager.createRoom(roomId);
    ...
    private void onReceiveJoinRoom(String received) {
        ...
        String roomId = splitted[1];
        String result = "success";
        System.out.println("Received: " + roomId + result);
    }
